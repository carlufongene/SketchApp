...
public class EditorOne extends JFrame {
	...
	private String shapeType = "polyline" ;        // type of object to add
	...
	private JComponent setupGUI() {
		...
		String[] shapes = {"ellipse","segment","polyline","rectangle"};
	...
	private void handlePress(Point p) {
		if (mode == Mode.DRAW) {
			...
			if (shapeType.equals("segment")) {
				segment = new Segment(p.x,p.y,color);
				drawFrom = p;
			}
			if (shapeType.equals("polyline")) {
				polyline = new Polyline(p,color);
				drawFrom = p;
			}
			if (shapeType.equals("rectangle")) {
				rectangle = new Rectangle(p.x, p.y, color);
				drawFrom = p;
	...
	private void handleDrag(Point p) {
		if (mode == Mode.DRAW) {
			...
			if (shapeType == "segment") {
				segment.setStart(drawFrom.x, drawFrom.y);
			...
			if (shapeType == "rectangle") {
				rectangle.setCorners(drawFrom.x,drawFrom.y,p.x, p.y);
			...
			if (shapeType == "segment") {
				if (moveFrom != null) {
					segment.moveBy(p.x - moveFrom.x, p.y - moveFrom.y);
					moveFrom = p;
				}
			}
			if (shapeType == "polyline") {
				if (moveFrom != null) {
					polyline.moveBy(p.x - moveFrom.x, p.y - moveFrom.y);
					moveFrom = p;
				}
			}
			if (shapeType == "rectangle") {
				if (moveFrom != null) {
					rectangle.moveBy(p.x - moveFrom.x, p.y - moveFrom.y);
					moveFrom = p;
				}
			}
